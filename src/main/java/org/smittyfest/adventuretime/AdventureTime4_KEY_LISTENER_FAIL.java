 ...
 * @author smittyfest
 */
public class AdventureTime4_KEY_LISTENER_FAIL extends Application {
  ...
  public void start(Stage primaryStage) throws Exception {
    ...
    final Tile pcTile = Tile.from("-", pcX, pcY);
    ...
    scene.setOnKeyPressed(new EventHandler<KeyEvent>() {
      public void handle(KeyEvent event) {
        switch (event.getCode()) {
          case UP:    pcTile.setYCoord(goNorth(pcTile)); break;
//          case DOWN:  goSouth = true; break;
//          case LEFT:  goWest  = true; break;
//          case RIGHT: goEast  = true; break;
//          case SHIFT: running = true; break;
        }
        board.add(pcTile, Integer.valueOf(pcTile.getYCoord()), pcTile.getXCoord());
      }
    });
  }

  private double goNorth(Tile pc) {
    double y = pc.getYCoord();
    if (y > 0) {
      y -= 1;
    }
    return y;
